...
public class IndexService {
	...
	public static final String CATEGORY_CACHE_KEY_PREFIX = "index:category:level:";
	...
	public List<Category> queryLevel1Categories() {
		...
		// 从缓存中获取
		String cacheCategories = this.redisTemplate.opsForValue().get(CATEGORY_CACHE_KEY_PREFIX + 1);
		...
		// 如果缓存中没有则调用远程接口获取
		Resp<List<Category>> listResp = this.pmsFeign.queryCategoryByPidOrLevel(0L, 1);
		List<Category> categoryEntities = listResp.getData();

		// 把查询结果放入缓存
		this.redisTemplate.opsForValue().set(CATEGORY_CACHE_KEY_PREFIX + 1, JSON.toJSONString(categoryEntities));
